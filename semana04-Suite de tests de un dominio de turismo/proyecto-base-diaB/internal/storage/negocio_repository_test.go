...
func TestNegocioMemoria_Eliminar(t *testing.T) {
	...
	for _, c := range casos {
		t.Run(c.nombre, func(t *testing.T) {
			repo := NewNegocioMemoria()
			repo.Guardar(base) // Sembramos el dato
...
func TestNegocioMemoria_BuscarPorID_Errores(t *testing.T) {
	repo := NewNegocioMemoria()
	...
	casos := []struct {
		...
		{"ID negativo falla", -1, errs.ErrDatosInvalidos},
		{"ID inexistente falla", 404, errs.ErrNoEncontrado},
	}
...
func TestNegocioMemoria_Listar(t *testing.T) {
	repo := NewNegocioMemoria()
	repo.Guardar(models.Negocio{ID: 1, Nombre: "A", Tipo: "hotel", IdiomasHablados: []string{"es"}})
	repo.Guardar(models.Negocio{ID: 2, Nombre: "B", Tipo: "restaurante", IdiomasHablados: []string{"es"}})
